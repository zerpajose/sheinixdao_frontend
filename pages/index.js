...
export default function Home() {
  ...
  // hasToken keep track if the current address has the token
  ...
  const getProviderOrSigner = async (needSigner = false) => {
    ...
    if (chainId !== 69) {
      window.alert("Change the network to Kovan Optimism");
      throw new Error("Change network to Kovan Optimism");
    }
  ...
  const checkIfAddressInWhitelist = async () => {
    try {
      ...
      const _joinedWhitelist = await contract.allowList(address);
  ...
  /**
   * checkIfHasToken: Checks if the address already has the token
   */
   const checkIfHasToken = async () => {
    try {
      ...
      const _hasToken = await contract.balanceOf(address, 0);

      // Using the formatUnits function from ethers utils to format the result to int type and update react state
      if(utils.formatUnits(_hasToken, "wei") == 1){
        setHasToken(true);
      }
  ...
  */
  const renderButton = () => {
    if (walletConnected) {
      if (joinedWhitelist) {
        ...
        return (
          <div className={styles.description}>
            In order to join the whitelist please go to Sheinix Discord!
          </div>
        );
      }
    } else {
  ...
  return (
    <div>
      <Head>
        <title>Sheinix DAO</title>
        <meta name="description" content="Whitelist-Dapp" />
        <link rel="icon" href="/favicon.ico" />
      </Head>
      <div className={styles.main}>
        <div>
          <h1 className={styles.title}>Welcome to Sheinix DAO</h1>
          <div className={styles.description}>
